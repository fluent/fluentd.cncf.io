...
Find.find("#{top_dir}/#{options[:channel]}") do |path|
  ...
  Dir.chdir(path) do
    ...
    relative_paths = []
    paths.each_with_index do |dir, index|
      target_path = File.join(paths[0..index].join('/'), 'index.html')
      entry = {label: dir, path: Pathname.new(target_path).relative_path_from(relative_path).to_s}
