...
class FluentInstallScript
  ...
  def fetch_previous_install_script(install_script)
    ...
    if @options[:site] == "https://fluentd.cdn.cncf.io"
    ...
    @metadata.keys.each do |key|
      ...
      if @options[:verify]
        ...
        if previous_content != content # ignore newline
          ...
          puts "[NG]".red.on_black + " Compared with: #{previous_script_path}"
        else
          puts "[OK]".green.on_black + " Compared with: #{previous_script_path}"
        end
      end
    end
  end
